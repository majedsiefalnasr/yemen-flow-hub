...
return [
    ...
    // above legitimate dashboard/queue polling (each page load fans out multiple
    // parallel GETs, so 120 starved fast navigation). Tighter per-route throttles
    // (uploads, admin writes, auth) stack on top of this and still win.
    'api_throttle_per_minute' => (int) env('API_THROTTLE_PER_MINUTE', 240),
