...
class ReportController extends Controller
{
    public function __construct(
        ...
        private readonly ReportAggregateCache $aggregateCache,
    ) {}
    ...
    public function summary(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('summary', $request, function () use ($request) {
            ...
            return compact('total', 'active', 'closed', 'rejected', 'cancelled', 'abandoned', 'totalAmount');
        });
    ...
    public function requestsOverTime(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('requests-over-time', $request, function () use ($request) {
    ...
    public function byWorkflowStage(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('by-workflow-stage', $request, function () use ($request) {
            $query = $this->baseQuery($request);
    ...
    public function byBank(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('by-bank', $request, function () use ($request) {
            $query = $this->baseQuery($request);
    ...
    public function byMerchant(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('by-merchant', $request, function () use ($request) {
            $query = $this->baseQuery($request);
    ...
    public function bySector(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('by-sector', $request, function () use ($request) {
            $query = $this->baseQuery($request);
    ...
    public function byCurrency(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('by-currency', $request, function () use ($request) {
            $query = $this->baseQuery($request);
    ...
    public function stageDuration(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('stage-duration', $request, function () use ($request) {
    ...
    public function sla(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('sla', $request, function () use ($request) {
    ...
    public function teamPerformance(Request $request): JsonResponse
    {
        ...
        $data = $this->cached('team-performance', $request, function () use ($request) {
    ...
    /**
     * CACHE-001: read-through cache for a pure aggregate endpoint. The scope
     * key is derived from the same DataScope::forUser() resolution already
     * used for query-level scoping, so a cached entry can never be shared
     * across banks or keyed on the raw user id.
     */
    private function cached(string $endpoint, Request $request, \Closure $compute): mixed
    {
        $scope = DataScope::forUser($request->user());

        return $this->aggregateCache->remember($endpoint, $scope, $request->query(), $compute);
    }
