...
- [ ] **DB-001:** EXPLAIN shows covering index lookup on the SLA subquery; my-queue p95 ≤ 300 ms at 1M rows. *(Index + code done — DB-001 index applied and ARCH-002 `stage_entered_at` projection column swapped in; `SlaProjectionParityTest`, `evidence/explain/ARCH-002-stage-entered-at.txt`. **Load-run harness executed twice this pass** (`perf:load-scenario`, real MySQL dev DB): first run — p95 held at 1K rows (42ms) and 50K rows (181ms) but failed at 200K rows (574ms). **Follow-up attempted**: added a maintained, indexed `sla_deadline_epoch` column (mirrors ARCH-002's `stage_entered_at` pattern) so the ORDER BY reads a plain indexed column instead of a computed expression — genuinely fixes the single-accessible-stage case (confirmed via EXPLAIN: index used, no filesort) but the realistic multi-accessible-stage case still filesorts at 554ms: MySQL cannot use an index for both a multi-value `current_stage_id IN (...)` filter and an ORDER BY simultaneously (a documented MySQL optimizer limitation, verified precisely via EXPLAIN with 1 vs 2 stage IDs). `evidence/DB-001-002-sla-deadline-column-followup.md` recommends a UNION-per-stage query restructure as the real next step. Also surfaced a new, separate, higher-priority finding: **TZ-001** (`evidence/TZ-001-mysql-timestamp-session-timezone-bug.md`) — MySQL `TIMESTAMP` columns are stored 3 hours off from the true UTC instant on production, discovered while verifying this column's backfill. Gate stays open.)*
- [ ] **DB-002 + ARCH-004:** EXPLAIN shows covering range scan (no `cast(... as date)` in the plan); list p95 ≤ 300 ms. *(Code + index done — `er_bank_created` index and half-open date range in `EngineRequestListQuery` (no `whereDate`); `EngineSearchTest` covers the range boundaries. **Load-run harness executed**: p95 held at 1K (40ms) and 50K (112ms) but failed at 200K rows (367ms then, still ~374ms after the DB-001 follow-up above — same MySQL IN+ORDER BY limitation). Gate stays open.)*
- [ ] **TZ-001 (new):** MySQL session timezone matches `config('app.timezone')`; historical `TIMESTAMP` data verified/corrected for the pre-existing 3-hour skew; SLA breach classification re-verified against real data before/after. *(Not started — investigated and reverted this pass, see DB-001's entry and `evidence/TZ-001-mysql-timestamp-session-timezone-bug.md`. Needs its own dedicated session: full column inventory, backfill-vs-column-type-change decision, migration with rollback.)*
