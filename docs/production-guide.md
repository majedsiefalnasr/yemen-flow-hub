...
| ------------------------------- | ------------ | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `workflow:expire-engine-claims` | Every minute | Release support claims past TTL (`claim_expires_at`); notify CBY admins  | `workflow.support_claim_ttl_minutes` — this is the **config key**, not the runtime TTL source; the live TTL is the admin `support_claim_ttl` setting read via `EngineClaimService::ttlMinutes()`'s `SettingResolver` dependency (see [`architecture/02-workflow-engine.md`](architecture/02-workflow-engine.md)) |
