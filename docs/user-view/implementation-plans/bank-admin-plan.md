...
- [ ] Verify sidebar renders dashboard, requests, notifications, merchants, staff, reports, and settings.
- [ ] Keep New Request out of primary nav and dashboard primary actions.
- [ ] If `/requests/new` remains reachable as fallback, make it low-prominence and guarded by clear requester identity.
- [ ] Hide all approve, return, reject, claim, vote, SWIFT, and FX completion controls.
...
- [ ] Implement tabs: pending, at_cby, swift_fx, completed, rejected, all.
- [ ] Header includes read-only oversight chip and no New Request CTA.
- [ ] Toolbar includes search, column visibility, export, saved views, and refresh.
- [ ] Limit bulk toolbar to export/print/clear.
- [ ] Table shows Created By, Current Owner, Age in Stage, and View-only action.
- [ ] Request detail shares reviewer read-only layout but actions panel contains only current owner/stage text.
- [ ] For drafts created by current admin fallback, render edit draft and delete draft only.
- [ ] Documents tab allows request docs, SWIFT, and FX request downloads but shows external FX confirmation as locked row.
- [ ] Never render decision buttons or disabled workflow buttons.

### 4. Staff Management

- [ ] Treat `/staff` as own-bank IAM, not HR.
- [ ] Add access health summary cards: active staff, MFA enabled percent, inactive, critical role coverage, recent role changes, recent permission denials.
- [ ] Add filters: search, role, status, MFA, last-login range.
- [ ] Staff table columns: employee, role, status, MFA, last login, workload, created date, actions.
- [ ] Restrict role select to `DATA_ENTRY` and `BANK_REVIEWER`.
- [ ] Do not render CBY roles or `SWIFT_OFFICER` as assignable.
- [ ] Use shadcn-vue Dialog for add/edit and AlertDialog for deactivation/reset/force logout.
...
- [ ] Build bank-scoped merchant registry focused on completeness and duplicate prevention.
- [ ] Add quality summary: total, active, incomplete records, possible duplicates, inactive.
- [ ] Add filters: search, status, completeness.
- [ ] Table columns: name, registry number, tax ID, status, linked requests, last activity, completeness, actions.
- [ ] Add/edit dialog validates required fields and uniqueness within bank.
- [ ] Duplicate warnings require explicit confirmation to proceed.
- [ ] Empty state includes add merchant CTA.

### 6. Reports, Notifications, Settings, Profile

- [ ] Reports remain own-bank only and lighter than CBY reports.
- [ ] Add KPI strip, monthly trend, category donut, currency bar chart, submission heatmap, outcomes table, and scheduled reports if backend supports it.
- [ ] Notifications are rollups only: staff lifecycle, merchant alerts, bank operational concerns, scheduled reports.
- [ ] Exclude per-request workflow noise by default.
- [ ] Profile stats highlight staff managed, merchants managed, and last admin action.

### 7. Backend And Data Readiness

- [ ] Confirm `/api/dashboard/stats` supports bank admin extended metrics or add fields safely.
- [ ] Confirm `UserController` role assignment validation blocks CBY roles and `SWIFT_OFFICER` for bank admins.
- [ ] Confirm staff list filters own bank and excludes CBY users.
- [ ] Confirm merchant CRUD is own-bank scoped and duplicate detection supports warning metadata.
- [ ] Confirm reports are bank-scoped for bank admins.
