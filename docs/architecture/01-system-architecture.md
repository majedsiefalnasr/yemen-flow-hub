...
operational workflow platform. All users inside the same bank can view
all requests belonging to their bank, but actions remain
capability-scoped, and dashboards remain queue/operationally scoped, not
role-name-branched.

The system enforces: organization-scoped visibility, capability-scoped
operational actions, queue-based operational workflows, workflow-aware
request filtering. See
...
the full schema, verified directly against migrations.
...
Each `engine_requests` row tracks: current stage (`current_stage_id`,
expressing business status — not a separate status column), organizational
workflow ownership (`bank_id`), workflow history (via `workflow_history`),
documents, audit events, and claim lifecycle. There is no vote tracking —
see the database doc's "Legacy tables" section for what was dropped.
