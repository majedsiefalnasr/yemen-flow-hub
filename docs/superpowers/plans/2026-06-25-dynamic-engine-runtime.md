...
- Every backend change committed to both `backend/` (team repo) and root monorepo; every frontend change committed to both `frontend/` (team repo) and root monorepo, per `AGENTS.md`. Conventional commit format `type(scope): description`, signed, `Co-Authored-By: Claude <noreply@anthropic.com>`.
