...
- Remaining failures (all confirmed pre-existing, outside this plan's target clusters — none touch a file modified by Tasks 2-8):
  - `Tests\Feature\Profile\ProfileControllerTest` — 6 failures (`change password with...`, `change password logs...`, `change password rejects...`, `put profile updates...`, `post mfa toggle when setting not registered`, and a `UniqueConstraintViolationException` on `system_settings.key` in `post mfa toggle returns 403 when system enforced`). Reproduces in isolation (`php artisan test tests/Feature/Profile/ProfileControllerTest.php`), so this is not test-order pollution from another file — state appears to leak between tests within this file's own run (a `SystemSetting` row created by one test is still present when a later test tries to create the same key). Root cause not fully traced; candidate is a `RefreshDatabase`/in-memory-SQLite isolation gap specific to this file, not a pivot-role or governance-identity fixture gap (the failing assertions do not touch `role`, `organization_id`, or governance pivots).
