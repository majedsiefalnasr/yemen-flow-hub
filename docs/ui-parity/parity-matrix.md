...
| area | lovable screenshot path | current Vue path | viewport | verdict | gap summary | remediation story | posture | visual_baseline |
| ---- | ----------------------- | ---------------- | -------- | ------- | ----------- | ----------------- | ------- | --------------- |
...
| dashboards/data-entry | `lovable/screenshots/DATA_ENTRY/dashboard.png` | `frontend/app/pages/dashboard.vue` (DATA_ENTRY variant) | desktop+mobile | PASS | KPI grid (4), Quick Actions row (3), alert banner, dual data tables — structure matches Lovable cleanly. Spacing and color tokens match. RTL OK across all five probes. | n/a | n/a | LOCKED |
