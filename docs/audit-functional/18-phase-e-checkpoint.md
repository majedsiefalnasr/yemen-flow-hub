...
| E9 | 17-finding traceability matrix | [`17-phase-e-traceability-matrix.md`](./17-phase-e-traceability-matrix.md) — all 16 tracked findings mapped to fix commit + automated test + verification + status (all ✅ RESOLVED), plus 1 new finding (E8-001) discovered and resolved in this same phase, plus a 6-entry baseline-failure registry with stable IDs (`BASELINE-FE-001..004`, `BASELINE-BE-001..002`, `BASELINE-FMT-001`). |
...
| `docs/audit-functional/17-phase-e-traceability-matrix.md` | New file: 17-finding matrix + baseline registry |
