...
> **Archived — historical structure only, not current testing authority.**
> This suite predates the dynamic workflow engine. It describes a fixed
> status pipeline (`SUBMITTED`, `EXECUTIVE_VOTING_OPEN`,
> `CUSTOMS_DECLARATION_ISSUED`, etc.) and an executive voting flow that no
> longer exist — see
> [`docs/architecture/05-request-state-model.md`](../../architecture/05-request-state-model.md)
> for the current four-field request state model, and
> [`AGENTS.md`](../../../AGENTS.md) for why Executive Voting is out of V1.
> Do not use its workflow/status/permission content as current. It is
> retained only for its structural scaffolding (test-user-alias table,
> evidence template, exit criteria) as raw material for
> `docs/testing-guide.md` (planned, Step 12).
