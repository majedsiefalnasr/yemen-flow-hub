  ...
  switch (role) {
    case 'DATA_ENTRY':
      return {
        ...
        completed: 12,
        draft_requests: [makeRequest(1, 'DRAFT')],
        returned_requests: [req3],
        recent_requests: [req1, req2],
      }
    case 'BANK_REVIEWER':
      return {
        pending_review: 4,
        ...
        review_queue: [req2, makeRequest(1008, 'BANK_REVIEW')],
        downstream_queue: [req4],
      }
    case 'BANK_ADMIN':
      return {
        total: 25,
    ...
    case 'SUPPORT_COMMITTEE':
      return {
        ...
        claimed_by_others: 2,
        recently_approved: 5,
        support_queue: [req4, makeRequest(1009, 'SUPPORT_REVIEW_PENDING')],
      }
    case 'SWIFT_OFFICER':
      return {
        pending_swift_upload: 2,
        uploaded: 1,
        final_approved: 7,
        final_rejected: 1,
        swift_queue: [req5, makeRequest(1010, 'SWIFT_UPLOADED')],
      }
    case 'EXECUTIVE_MEMBER':
      return {
        ...
        finalized_decisions: 17,
        pending_my_vote: 2,
        voting_queue: [{ ...req6, my_vote: null, votes_cast: 3, total_voters: 6 }],
      }
    case 'COMMITTEE_DIRECTOR':
      return {
        ...
        finalized_approved: 14,
        finalized_rejected: 3,
        voting_lifecycle_queue: [
          { ...req6, my_vote: null, votes_cast: 6, total_voters: 6, ready_to_close: true },
        ],
        fx_confirmation_queue: [req7],
      }
    case 'CBY_ADMIN':
      return {
        total_requests: 158,
        ...
        sla_breach_count: 2,
        monthly_trend: [
          { month: 'مارس', submitted: 25, approved: 22 },
          { month: 'أبريل', submitted: 30, approved: 27 },
          { month: 'مايو', submitted: 18, approved: 14 },
        ],
        category_distribution: [
          { label: 'مواد غذائية', count: 45, color: '#0066cc' },
          { label: 'أدوية', count: 28, color: '#5856d6' },
        ],
...
export async function mockApi(page: Page, role: VisualRole) {
  ...
  await page.route('**/sanctum/csrf-cookie', async (route) => {
    ...
    await route.fulfill({
      status: 204,
      headers: { ...corsHeaders, 'set-cookie': 'XSRF-TOKEN=test-xsrf; Path=/' },
      body: '',
    })
  })
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/auth/me') {
      await json(route, apiOk(user))
      return
    }
    if (path === '/api/auth/login') {
      await json(
        route,
        apiOk({ user, token: null, token_type: null, mode: 'cookie', requires_mfa: false }),
      )
      return
    }
    if (path === '/api/dashboard/stats') {
      await json(route, apiOk(dashboardStats(role)))
      return
    }
    if (path === '/api/requests') {
      await json(
        route,
        paginatedOk([
          makeRequest(2001, 'SUBMITTED'),
          makeRequest(2002, 'BANK_REVIEW'),
          makeRequest(2003, 'BANK_APPROVED'),
          makeRequest(2004, 'SUPPORT_REVIEW_PENDING'),
        ]),
      )
      return
    }
    if (path.match(/^\/api\/requests\/\d+$/)) {
