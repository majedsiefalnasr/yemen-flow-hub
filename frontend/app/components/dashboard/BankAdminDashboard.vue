    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-4 max-lg:grid-cols-2 max-md:grid-cols-1 gap-4">
        <template v-for="kpi in kpiConfig" :key="kpi.label">
          <Card class="border-0 p-4 shadow-card flex flex-col gap-1.5" :class="{ 'border-s-4 border-s-warning': kpi.variant === 'amber' }">
            ...
            <span class="text-2xl font-semibold leading-none" :class="kpi.variant === 'amber' && kpi.value > 0 ? 'text-warning' : kpi.variant === 'green' ? 'text-success' : kpi.variant === 'indigo' ? 'text-indigo-600' : 'text-foreground'">
              {{ kpi.value }}
            </span>
      ...
      <Card class="border-0 shadow-card" aria-labelledby="recent-heading">
        <CardContent class="p-4">
          ...
          <table v-else class="w-full border-collapse text-xs" role="table" aria-label="أحدث طلبات البنك">
            ...
            <tbody>
              <tr v-for="req in stats.recent_requests" :key="req.id" class="border-t border-muted hover:bg-muted cursor-pointer transition-colors">
