...
<template>
  <div class="flex flex-col gap-6" dir="rtl">
    ...
    <div v-if="store.loading" class="grid grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4" aria-busy="true" aria-label="جارٍ تحميل الإحصائيات">
      <div v-for="n in 3" :key="n" class="bg-background border border-border rounded-md p-6 flex flex-col gap-1.5 animate-pulse" aria-hidden="true">
    ...
    <!-- Error -->
    <div v-else-if="store.error" class="border border-border rounded-md p-5 text-error-text flex items-center gap-3" role="alert">
    ...
    <template v-else-if="stats">
      ...
      <div :class="isDirector ? 'grid grid-cols-4 lg:grid-cols-2 md:grid-cols-1 gap-4' : 'grid grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4'">
        ...
        <div class="bg-background border border-border rounded-md p-6 flex flex-col gap-1.5">
          ...
          <span class="text-xs text-muted-foreground">قرارات رفض</span>
        </div>
        ...
        <div class="bg-background border border-border rounded-md p-6 flex flex-col gap-1.5">
          ...
          <span class="text-xs text-muted-foreground">قرارات اعتماد</span>
        </div>

        <!-- طابور التصويت -->
        <div class="bg-background border rounded-md p-6 flex flex-col gap-1.5" :class="stats.active_voting_sessions > 0 ? 'border-voting-indigo border-l-4' : 'border-border'">
          ...
          <span class="text-xs text-muted-foreground">طابور التصويت</span>
        </div>
        ...
        <div v-if="isDirector" class="bg-white border border-l-4 border-warning-text rounded-md p-6 flex flex-col gap-1.5">
          ...
          <span class="text-xs text-muted-foreground">تجاوزات المدير</span>
        </div>
      </div>
      ...
      <section aria-labelledby="qa-heading">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-1">
          ...
          <button class="flex flex-col items-start gap-1 p-5 bg-background border border-border rounded-md hover:border-primary hover:text-primary transition-colors" @click="router.push('/reports')">
            <div class="w-8 h-8 flex items-center justify-center text-primary mb-1" aria-hidden="true">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" />
              </svg>
            </div>
            <span class="text-sm font-semibold text-foreground">التقارير</span>
      ...
      <section aria-labelledby="voting-queue-heading">
        ...
        <div v-if="(stats.voting_queue?.length ?? 0) === 0" class="bg-background border border-border rounded-md p-10 flex flex-col items-center gap-3 text-muted-foreground text-sm" role="status">
        ...
        <table v-else class="w-full border-collapse text-xs bg-background border border-border rounded-md overflow-hidden" role="table" aria-label="طلبات بانتظار تصويتك">
          <thead>
            <tr class="bg-muted">
              ...
              <th class="px-3.5 py-2.5 text-right font-medium text-muted-foreground border-b border-border whitespace-nowrap">��جراء</th>
            </tr>
          </thead>
          <tbody>
            <tr
              ...
              :key="req.id"
              class="hover:bg-muted/50 border-t border-border"
              :class="{ 'bg-indigo-50': isVotingOpen(req.status) }"
            >
              <td class="px-3.5 py-2.5"><a class="text-primary hover:underline font-mono" :href="`/requests/${req.id}`" @click.prevent="router.push(`/requests/${req.id}`)">{{ req.reference_number }}</a></td>
              <td class="px-3.5 py-2.5 text-right">{{ req.supplier_name }}</td>
              <td class="px-3.5 py-2.5 text-start direction-ltr font-variant-numeric-tabular-nums">{{ formatAmount(req.amount, req.currency) }}</td>
              ...
              <td class="px-3.5 py-2.5">
                ...
                <span v-else class="inline-flex items-center px-2.5 py-1 bg-muted text-muted-foreground rounded-full text-xs">انتظار فتح التص��يت</span>
              </td>
              <td class="px-3.5 py-2.5"><button class="px-3.5 py-1.5 bg-background border border-border rounded text-xs hover:border-primary hover:text-primary transition-colors" :aria-label="`عرض الطلب ${req.reference_number}`" @click="router.push(`/requests/${req.id}`)">عرض</button></td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <section v-if="isDirector" aria-labelledby="customs-heading">
        ...
        <div v-if="customsDeclarationPending.length === 0" class="bg-background border border-border rounded-md p-10 flex flex-col items-center gap-3 text-muted-foreground text-sm" role="status">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
            <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" />
          </svg>
          <p>لا توجد بيانات جمركية بانتظار الإصدار حالياً</p>
        </div>
        <table v-else class="w-full border-collapse text-xs bg-background border border-border rounded-md overflow-hidden" role="table" aria-label="طلبات بانتظار إصدار البيان الجمركي">
          <thead>
            <tr class="bg-muted">
              <th class="px-3.5 py-2.5 text-right font-medium text-muted-foreground border-b border-border whitespace-nowrap">المرجع</th>
          ...
          <tbody>
            <tr v-for="req in customsDeclarationPending" :key="req.id" class="hover:bg-muted/50 border-t border-border">
              <td class="px-3.5 py-2.5"><a class="text-primary hover:underline font-mono" :href="`/requests/${req.id}`" @click.prevent="router.push(`/requests/${req.id}`)">{{ req.reference_number }}</a></td>
              ...
              <td class="px-3.5 py-2.5"><button class="px-3.5 py-1.5 bg-primary text-primary-foreground rounded text-xs hover:opacity-90 transition-opacity" :aria-label="`إصدار البيان الجمركي للطلب ${req.reference_number}`" @click="router.push(`/requests/${req.id}`)">إصدار</button></td>
            </tr>
          </tbody>
        </table>
      </section>
