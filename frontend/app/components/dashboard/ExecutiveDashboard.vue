    ...
    <template v-else-if="stats">
      ...
      <div :class="isDirector ? 'grid grid-cols-4 lg:grid-cols-2 md:grid-cols-1 gap-4' : 'grid grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4'">
        ...
        <div class="bg-white border border-border rounded-md p-6 flex flex-col gap-1.5">
          ...
          <span class="text-2xl font-semibold text-success-text">{{ isDirector ? stats.finalized_decisions : stats.decisions_approved }}</span>
          <span class="text-xs text-muted-foreground">قرارات اعتماد</span>
        </div>

        <!-- طابور التصويت -->
        <div class="bg-white border rounded-md p-6 flex flex-col gap-1.5" :class="stats.active_voting_sessions > 0 ? 'border-voting-indigo border-l-4' : 'border-border'">
        ...
        <div v-if="isDirector" class="bg-white border border-l-4 border-warning-text rounded-md p-6 flex flex-col gap-1.5">
          <div class="w-9 h-9 rounded bg-yellow-50 text-warning-text flex items-center justify-center mb-1" aria-hidden="true">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
            </svg>
          </div>
          <span class="text-2xl font-semibold text-warning-text">{{ (stats as any).director_override_count ?? 0 }}</span>
      ...
      <section aria-labelledby="qa-heading">
        <h2 id="qa-heading" class="flex items-center gap-2 text-sm font-semibold text-primary-text mb-3">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-1">
          ...
          <button class="flex flex-col items-start gap-1 p-5 bg-white border border-border rounded-md hover:border-primary-blue hover:text-primary-blue transition-colors" @click="router.push('/reports')">
            <div class="w-8 h-8 flex items-center justify-center text-primary-blue mb-1" aria-hidden="true">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" />
              </svg>
            </div>
            <span class="text-sm font-semibold text-primary-text">التقارير</span>
      ...
      <section aria-labelledby="voting-queue-heading">
        <div class="flex items-center justify-between mb-3">
          <h2 id="voting-queue-heading" class="text-sm font-semibold text-primary-text">طلبات بانتظار تصويتك</h2>
          <a class="text-xs text-primary-blue hover:underline" href="/requests" @click.prevent="router.push('/requests')">عرض الكل</a>
        </div>

        <div v-if="(stats.voting_queue?.length ?? 0) === 0" class="bg-white border border-border rounded-md p-10 flex flex-col items-center gap-3 text-muted-foreground text-sm" role="status">
      ...
      <section v-if="isDirector" aria-labelledby="customs-heading">
        <div class="flex items-center justify-between mb-3">
          <h2 id="customs-heading" class="text-sm font-semibold text-primary-text">بيانات جمركية بانتظار الإصدار</h2>
        </div>
        <div v-if="customsDeclarationPending.length === 0" class="bg-white border border-border rounded-md p-10 flex flex-col items-center gap-3 text-muted-foreground text-sm" role="status">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
            <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" />
          </svg>
        ...
        <table v-else class="w-full border-collapse text-xs bg-white border border-border rounded-md overflow-hidden" role="table" aria-label="طلبات بانتظار إصدار البيان الجمركي">
          ...
          <tbody>
            <tr v-for="req in customsDeclarationPending" :key="req.id" class="hover:bg-muted border-t border-border">
              ...
              <td class="px-3.5 py-2.5"><button class="px-3.5 py-1.5 bg-primary-blue text-white rounded text-xs hover:opacity-90 transition-opacity" :aria-label="`إصدار البيان الجمركي للطلب ${req.reference_number}`" @click="router.push(`/requests/${req.id}`)">إصدار</button></td>
            </tr>
          </tbody>
        </table>
      </section>
