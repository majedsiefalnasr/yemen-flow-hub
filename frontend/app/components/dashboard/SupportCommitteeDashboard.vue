    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-4 max-lg:grid-cols-2 max-md:grid-cols-1 gap-4">
        <template v-for="kpi in kpiConfig" :key="kpi.label">
          <Card class="border-0 p-4 shadow flex flex-col gap-1.5" :class="{ 'border-s-4 border-s-amber-600': kpi.variant === 'amber', 'border-s-4 border-s-indigo-600': kpi.variant === 'indigo' }">
            ...
            <span class="text-2xl font-semibold leading-none" :class="kpi.variant === 'amber' && kpi.value > 0 ? 'text-amber-600' : kpi.variant === 'indigo' && kpi.value > 0 ? 'text-indigo-600' : kpi.variant === 'green' ? 'text-green-700' : 'text-gray-900'">
              {{ kpi.value }}
            </span>
      ...
      <section aria-labelledby="qa-heading">
        ...
        <div class="grid grid-cols-2 max-md:grid-cols-1 gap-3">
          ...
          <button class="flex flex-col items-start gap-1 p-4 bg-white border border-gray-200 text-gray-900 rounded-lg cursor-pointer hover:border-primary hover:shadow-md transition-all" @click="router.push('/notifications')">
            <Mail class="h-5 w-5 flex-shrink-0 text-blue-600 mb-1" aria-hidden="true" />
            <span class="text-sm font-semibold">الإشعارات</span>
            <span class="text-xs text-gray-600">آخر تحديثات الطابور والقرارات</span>
          </button>
        </div>
      </section>
      ...
      <Card class="border-0 shadow" aria-labelledby="queue-heading">
        <CardContent class="p-4">
          <div class="flex items-center justify-between mb-4">
            <h2 id="queue-heading" class="text-sm font-semibold text-gray-900">طابور عملي</h2>
            <a class="text-xs text-blue-600 hover:underline transition-colors cursor-pointer" @click="router.push('/requests')">عرض الكل</a>
          </div>

          <div v-if="queue.length === 0" class="py-8 text-center text-sm text-gray-600" role="status">لا توجد طلبات في طابور لجنة الدعم حالياً</div>
          ...
          <table v-else class="w-full border-collapse text-xs" role="table" aria-label="طابور عملي">
            <thead>
              <tr class="border-b border-gray-200">
                ...
                <th scope="col" class="py-2 px-2 text-right font-medium text-gray-600">الحجز</th>
            ...
            <tbody>
              <tr
                ...
                :class="{
                  'bg-blue-600/10 hover:bg-blue-600/10': req.is_claimed_by_me,
                ...
                <td class="py-2 px-2">
                  <span
                    ...
                    :class="{
                      ...
                      'bg-amber-50/10 text-amber-600': !!req.claimed_by && !req.is_claimed_by_me,
                      'bg-gray-50 text-gray-900': !req.claimed_by,
                    }"
                  >
                    {{ claimOwnerLabel(req) }}
                  </span>
