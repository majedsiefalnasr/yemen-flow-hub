    ...
    <template v-else-if="stats">
      ...
      <section aria-labelledby="qa-heading">
        ...
        <div class="grid grid-cols-3 max-md:grid-cols-1 gap-3">
          <!-- إنشاء طلب جديد -->
          <button class="flex flex-col items-start gap-1 p-4 bg-primary text-primary-foreground border-0 rounded-lg cursor-pointer hover:hover:opacity-90 transition-colors" @click="router.push('/requests/new')">
          ...
          <!-- متابعة طلباتك -->
          <button class="flex flex-col items-start gap-1 p-4 bg-background border border-border text-foreground rounded-lg cursor-pointer hover:border-primary hover:shadow-md transition-all" @click="router.push('/requests')">
      ...
      <Card v-if="stats.returned_requests.length > 0" class="border-l-4 border-l-amber-600 border-0 border-b border-border bg-background" role="alert" aria-label="طلبات تحتاج تعديل">
