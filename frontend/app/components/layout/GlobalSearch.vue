    ...
    >
      ...
      <template v-if="inputValue.length === 0 && recentSearches.length > 0">
        ...
        <button
          ...
          :key="term"
          class="flex w-full items-center gap-2.5 px-3 py-2 text-start transition-colors hover:bg-muted"
          type="button"
      ...
      <template v-else-if="inputValue.length >= 2">
        ...
        <div v-if="hasResults" class="flex flex-wrap gap-1.5 border-b border-border px-3 py-2">
          <button
            ...
            :class="activeFilter === chip.key ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-transparent text-foreground hover:bg-muted'"
            type="button"
