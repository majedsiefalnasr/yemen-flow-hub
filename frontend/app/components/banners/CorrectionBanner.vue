...
<template>
  <Alert variant="destructive" dir="rtl" class="flex items-start gap-3 border-warning bg-warning/10">
    <AlertTriangle class="h-5 w-5 flex-shrink-0 mt-0.5 text-warning" aria-hidden="true" />
    <div class="flex flex-col gap-1 flex-1">
      ...
      <p v-if="variant === 'bank_returned' && reviewerComment" class="text-xs text-warning">
        تعليق المراجع: {{ reviewerComment }}
      </p>
      <p v-else-if="variant === 'support_returned' && supportComment" class="text-xs text-warning">
        تعليق لجنة المساندة: {{ supportComment }}
      </p>
      <p v-else-if="rejectionReason" class="text-xs text-warning">سبب الإرجاع: {{ rejectionReason }}</p>
    </div>
  </Alert>
</template>
