...
<template>
  <div class="max-w-[900px] mx-auto bg-white text-foreground p-6 print:p-0 print:max-w-full" dir="rtl">
    <!-- ─── Section 1: Title + reference + status ─── -->
    <header class="flex items-start justify-between gap-4 mb-2">
      <div class="flex flex-col gap-1">
        <h1 class="font-bold text-2xl text-foreground m-0">طلب تمويل واردات</h1>
        <span class="text-primary font-semibold text-base">{{ request.reference_number }}</span>
      </div>
      <span class="text-sm font-semibold px-3 py-1 rounded-full border border-border text-foreground bg-muted whitespace-nowrap flex-shrink-0 print:border-black print:bg-transparent print:text-black">{{ statusLabel(request.status) }}</span>
    </header>
    ...
    <section class="flex flex-col gap-1.5" aria-label="بيانات الطلب">
      <div class="flex gap-2 text-sm">
        <span class="font-semibold w-36 text-muted-foreground">البنك:</span>
        <span class="text-foreground">{{ request.bank_name ?? '—' }}</span>
      </div>
      <div class="flex gap-2 text-sm">
        <span class="font-semibold w-36 text-muted-foreground">مقدّم الطلب:</span>
        <span class="text-foreground">{{ actorName(request.created_by_user) }}</span>
      </div>
      <div class="flex gap-2 text-sm">
        <span class="font-semibold w-36 text-muted-foreground">تاريخ الإنشاء:</span>
        <span class="text-foreground">{{ formatDate(request.created_at) }}</span>
      </div>
    </section>
    ...
    <section aria-label="تفاصيل الطلب">
      ...
      <div class="grid grid-cols-2 gap-y-2.5 gap-x-6">
        <div v-for="field in fields" :key="field.label" class="flex flex-col gap-0.5">
          <span class="text-xs text-muted-foreground font-semibold">{{ field.label }}</span>
    ...
    <section aria-label="المستندات المرفقة">
      ...
      <p v-if="documents.length === 0" class="text-sm text-muted-foreground text-center py-4">لا توجد مستندات مرفقة.</p>
      <table v-else class="w-full border-collapse text-sm">
        <thead>
          <tr>
            <th class="text-right py-2 px-2.5 border border-border bg-muted font-semibold text-foreground print:bg-border">اسم الملف</th>
            <th class="text-right py-2 px-2.5 border border-border bg-muted font-semibold text-foreground print:bg-border">تاريخ الرفع</th>
            <th class="text-right py-2 px-2.5 border border-border bg-muted font-semibold text-foreground print:bg-border">رُفع بواسطة</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="doc in documents" :key="doc.id">
            <td class="text-right py-2 px-2.5 border border-border text-foreground">{{ doc.original_filename }}</td>
            <td class="text-right py-2 px-2.5 border border-border text-foreground">{{ formatDate(doc.uploaded_at) }}</td>
            <td class="text-right py-2 px-2.5 border border-border text-foreground">{{ doc.uploaded_by_name ?? '—' }}</td>
          </tr>
        </tbody>
      </table>
    </section>
    ...
    <section aria-label="مسار سير العمل">
      ...
      <p v-if="workflowEntries.length === 0" class="text-sm text-muted-foreground text-center py-4">لا توجد مراحل مسجّلة بعد.</p>
      <ol v-else class="list-none p-0 m-0 flex flex-col gap-2.5 print:break-inside-avoid">
        <li v-for="entry in workflowEntries" :key="entry.id" class="workflow-entry grid grid-cols-[1.4fr_1fr_auto] gap-3 items-center p-3 border border-border rounded-3xl bg-muted print:bg-transparent">
          <span class="text-sm font-semibold text-foreground">{{ entry.statusLabel }}</span>
          <span class="text-xs text-muted-foreground">{{ entry.actor }}</span>
          <span class="text-xs text-muted-foreground whitespace-nowrap">{{ entry.timestamp }}</span>
        </li>
      </ol>
    </section>
