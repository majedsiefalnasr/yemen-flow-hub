...
<template>
  <Card dir="rtl" class="hover:shadow-md transition-shadow">
    ...
    <div class="flex flex-col gap-1 py-3 border-b border-border">
      <p class="text-sm font-semibold text-foreground">{{ props.merchant.name }}</p>
      <p class="text-xs text-muted-foreground">{{ businessTypeLabel(props.merchant.business_type) }}</p>
    </div>

    <!-- Metadata rows -->
    <dl class="flex flex-col gap-2 py-3 border-b border-border text-xs">
      <div class="flex justify-between items-center gap-2">
        <dt class="text-muted-foreground">السجل التجاري</dt>
        <dd class="font-mono text-foreground font-medium">{{ metaVal(props.merchant.commercial_register) }}</dd>
      </div>
      <div class="flex justify-between items-center gap-2">
        <dt class="text-muted-foreground">الرقم الضريبي</dt>
        <dd class="font-mono text-foreground font-medium">{{ metaVal(props.merchant.tax_number) }}</dd>
      </div>
      <div class="flex justify-between items-center gap-2">
        <dt class="text-muted-foreground">البنك</dt>
        <dd class="text-foreground font-medium overflow-hidden text-ellipsis whitespace-nowrap max-w-xs">{{ metaVal(props.merchant.bank_name) }}</dd>
      </div>
      <div class="flex justify-between items-center gap-2">
        <dt class="text-muted-foreground">العنوان</dt>
        <dd class="text-foreground font-medium overflow-hidden text-ellipsis whitespace-nowrap max-w-xs">{{ metaVal(props.merchant.address) }}</dd>
      </div>
      <div class="flex justify-between items-center gap-2">
        <dt class="text-muted-foreground">هاتف</dt>
        <dd class="font-mono text-foreground font-medium direction-ltr">{{ metaVal(props.merchant.phone) }}</dd>
      </div>
    </dl>
    ...
    <div class="flex items-center justify-between gap-2 pt-3">
      <div class="text-xs flex items-center gap-1">
        ...
        <span class="font-mono font-bold text-foreground">{{ props.merchant.transaction_count ?? 0 }}</span>
      </div>
