...
const normalizedTrend = computed(
  () => props.trend ?? { direction: 'neutral' as const, value: '0%' },
)

const trendClass = computed(() => {
  if (normalizedTrend.value.direction === 'up')
    return 'bg-[color-mix(in_srgb,var(--severity-green)_12%,transparent)] text-[var(--severity-green)]'
  if (normalizedTrend.value.direction === 'down')
...
<template>
  <component :is="href ? 'a' : 'div'" :href="href" class="block" :data-testid="dataTestid">
    <Card
      class="flex flex-col gap-1.5 border-0 p-4 shadow transition-all"
      :class="[
