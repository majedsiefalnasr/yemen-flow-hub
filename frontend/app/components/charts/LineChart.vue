...
<template>
  <div class="flex flex-col gap-3" dir="rtl">
    <div v-if="!series.length || !labels.length || series.some((s) => s.values.length !== labels.length)" class="h-44 flex items-center justify-center text-muted-foreground text-sm">لا توجد بيانات</div>
    ...
    <div class="flex gap-4 justify-center flex-wrap">
      <div v-for="s in series" :key="`leg-${s.label}`" class="flex items-center gap-1.5 text-sm text-foreground">
