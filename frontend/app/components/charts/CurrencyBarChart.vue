...
<template>
  <div class="flex flex-col gap-2" dir="rtl">
    <div v-if="!data.length" class="h-28 flex items-center justify-center text-muted-foreground text-sm">لا توجد بيانات</div>
    <div v-else class="flex flex-col gap-3" role="list" aria-label="مخطط تمويل العملات">
      <div v-for="item in data" :key="item.currency" class="flex items-center gap-2.5" role="listitem">
        <span class="w-12 text-xs font-semibold text-foreground flex-shrink-0 text-end">{{ item.currency }}</span>
        ...
        <span class="w-13 text-xs text-muted-foreground flex-shrink-0 text-start font-tabular-nums">{{ formatAmount(item.amount) }}</span>
      </div>
    </div>
  </div>
</template>
