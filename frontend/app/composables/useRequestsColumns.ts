  ...
  const columns: ColumnDef<ImportRequest>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (request.duplicate_warnings?.length) {
          badges.push(
            h(Badge, { variant: 'destructive', class: 'rounded-full' }, () => [
        ...
        if (
          ...
          badges.push(
            h(Badge, { variant: 'secondary', class: 'rounded-full text-voting' }, () => [
              h(Vote, { class: 'size-3.5 me-1' }),
              'التصويت مفتوح',
            ]),
          )
        }
        const isBankReviewClaim =
          request.status === RequestStatus.BANK_REVIEW &&
          (role.value === UserRole.BANK_REVIEWER ||
            role.value === UserRole.BANK_ADMIN ||
            role.value === UserRole.CBY_ADMIN)
        const isSupportClaim = request.is_claimed && role.value === UserRole.SUPPORT_COMMITTEE
          ...
          h('div', { class: 'flex flex-wrap items-center gap-2' }, [
            h(
              'button',
              {
                type: 'button',
                class:
                  'font-mono text-base font-semibold text-primary underline-offset-2 hover:underline focus-visible:outline-none focus-visible:underline cursor-pointer',
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return h(
          ...
          days > 0 ? `${days} يوم` : `${Math.floor(hrs)} س`,
        )
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return h('div', { class: 'flex items-center gap-1.5' }, [
          ...
          h(
            'span',
            { class: pillClass(row.original.has_fx_request_document === true) },
            'طلب تأكيد المصارفة',
          ),
        ])
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (
          role.value === UserRole.BANK_REVIEWER &&
          [RequestStatus.SUBMITTED, RequestStatus.BANK_REVIEW].includes(request.status)
        ) {
          if (isBankReviewerSelf) {
            return h(
              ...
              {
                class:
                  'inline-flex rounded-md bg-muted px-2 py-1 text-xs text-muted-foreground cursor-not-allowed',
        ...
        if (role.value === UserRole.COMMITTEE_DIRECTOR) {
          const isReadyToClose =
            request.status === RequestStatus.EXECUTIVE_VOTING_OPEN && request.ready_to_close
          const isFxPending =
