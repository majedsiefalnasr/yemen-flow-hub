...
export function useApi() {
  ...
  function isCsrfMismatch(err: unknown): boolean {
    ...
    return candidate.status === 419 ||
      candidate.statusCode === 419 ||
      candidate.response?.status === 419
  }
  ...
  function buildHeaders(method: string, extraHeaders?: ApiFetchOptions['headers']): Record<string, string> {
  ...
  async function apiFetch<T>(
    ...
    if (process.client && isUnsafeMethod(method) && !getXsrfToken()) {
    ...
    try {
      ...
      if (process.client && isUnsafeMethod(method) && isCsrfMismatch(err)) {
        await ensureCsrfCookie()
        return request()
      }
      throw err
  ...
  async function post<T>(path: string, body?: ApiFetchBody, options: Omit<ApiFetchOptions, 'method' | 'body'> = {}): Promise<T> {
    return apiFetch<T>(path, { ...options, method: 'POST', body })
  }

  async function put<T>(path: string, body?: ApiFetchBody, options: Omit<ApiFetchOptions, 'method' | 'body'> = {}): Promise<T> {
