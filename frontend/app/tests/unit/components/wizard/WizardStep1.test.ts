...
describe('WizardStep1 — goods_type', () => {
  ...
  it('accepts valid type', () =>
    expect(validate({ ...VALID, goods_type: 'أدوية ومستلزمات طبية' }).success).toBe(true))
})
...
describe('WizardStep1 — amount', () => {
  ...
  it('accepts large amount', () =>
    expect(validate({ ...VALID, amount: 1_000_000 }).success).toBe(true))
})
...
describe('WizardStep1 — currency', () => {
  ...
  it('rejects unknown currency', () =>
    expect(errorsFor({ ...VALID, currency: 'GBP' })).toContain('currency'))
})

describe('WizardStep1 — payment_terms', () => {
  it('required', () =>
    expect(errorsFor({ ...VALID, payment_terms: '' })).toContain('payment_terms'))
  ...
  it('rejects unsupported backend values', () =>
    expect(errorsFor({ ...VALID, payment_terms: 'DP' })).toContain('payment_terms'))
})

describe('WizardStep1 — merchant_id', () => {
  it('required', () =>
    expect(errorsFor({ ...VALID, merchant_id: undefined })).toContain('merchant_id'))
  it('must be positive', () =>
    expect(errorsFor({ ...VALID, merchant_id: 0 })).toContain('merchant_id'))
})

describe('WizardStep1 — notes', () => {
  it('optional — accepts empty', () => expect(validate({ ...VALID, notes: '' }).success).toBe(true))
  it('optional — accepts null', () =>
    expect(validate({ ...VALID, notes: null }).success).toBe(true))
  it('max 500 chars', () =>
    expect(errorsFor({ ...VALID, notes: 'x'.repeat(501) })).toContain('notes'))
})

describe('WizardStep1 — due_date', () => {
  it('optional — accepts empty', () =>
    expect(validate({ ...VALID, due_date: '' }).success).toBe(true))
  it('optional — accepts null', () =>
    expect(validate({ ...VALID, due_date: null }).success).toBe(true))
  it('optional — accepts valid date string', () =>
    expect(validate({ ...VALID, due_date: '2027-01-01' }).success).toBe(true))
  ...
  it('rejects past dates', () =>
    expect(errorsFor({ ...VALID, due_date: '2020-01-01' })).toContain('due_date'))
})
