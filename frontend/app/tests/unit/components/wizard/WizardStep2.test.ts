...
describe('WizardStep2 — supplier_name', () => {
  it('required', () =>
    expect(errorsFor({ ...VALID, supplier_name: '' })).toContain('supplier_name'))
  it('accepts valid name', () => expect(validate(VALID).success).toBe(true))
  it('max 255 chars', () =>
    expect(errorsFor({ ...VALID, supplier_name: 'x'.repeat(256) })).toContain('supplier_name'))
})

describe('WizardStep2 — invoice_number', () => {
  it('required', () =>
    expect(errorsFor({ ...VALID, invoice_number: '' })).toContain('invoice_number'))
  it('max 100 chars', () =>
    expect(errorsFor({ ...VALID, invoice_number: 'x'.repeat(101) })).toContain('invoice_number'))
})

describe('WizardStep2 — origin_country', () => {
  it('required', () =>
    expect(errorsFor({ ...VALID, origin_country: '' })).toContain('origin_country'))
  it('accepts any non-empty string', () =>
    expect(validate({ ...VALID, origin_country: 'السعودية' }).success).toBe(true))
})
...
describe('WizardStep2 — invoice_date', () => {
  ...
  it('accepts past date', () =>
    expect(validate({ ...VALID, invoice_date: '2023-06-01' }).success).toBe(true))
  it('accepts future date', () =>
    expect(validate({ ...VALID, invoice_date: '2028-01-01' }).success).toBe(true))
})
...
describe('WizardStep2 — arrival_port', () => {
  ...
  it('accepts ميناء عدن', () =>
    expect(validate({ ...VALID, arrival_port: 'ميناء عدن' }).success).toBe(true))
})

describe('WizardStep2 — optional fields', () => {
  it('shipping_port optional', () =>
    expect(validate({ ...VALID, shipping_port: '' }).success).toBe(true))
  it('customs_office optional', () =>
    expect(validate({ ...VALID, customs_office: '' }).success).toBe(true))
