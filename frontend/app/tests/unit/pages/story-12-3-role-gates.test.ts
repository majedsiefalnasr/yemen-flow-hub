...
function swiftSubmitDisabledReason(
  swiftReference: string,
  hasSwiftFile: boolean,
  hasFxRequestFile: boolean,
): string {
...
function directorBanner(
  ...
  if (status === RequestStatus.EXECUTIVE_VOTING_OPEN && !readyToClose && !isTie)
...
function handleSwiftUploadError(message: string) {
  ...
  return {
    lockedStateError: '',
    uploadError: message || 'تعذّر تسليم وثائق السويفت. حاول مرة أخرى.',
  }
}
...
describe('Story 12.3 — Director composite strip', () => {
  it('maps all 3 sub-rows independently', () => {
    expect(
      directorCompositeRows({
        sessions_ready_to_close: 1,
        sessions_with_tie: 2,
        fx_confirmation_pending: 3,
      }),
    ).toEqual({
      readyToClose: true,
      tieBreak: true,
...
describe('Story 12.3 — Director controls', () => {
  it('gates close/finalize/override controls by status and vote completion', () => {
    expect(directorActions(RequestStatus.EXECUTIVE_VOTING_OPEN, false)).toEqual({
      canClose: false,
      canFinalize: false,
      canOverride: true,
    })
    expect(directorActions(RequestStatus.EXECUTIVE_VOTING_OPEN, true)).toEqual({
      canClose: true,
      canFinalize: false,
      canOverride: true,
    })
    expect(directorActions(RequestStatus.EXECUTIVE_VOTING_CLOSED, true)).toEqual({
      canClose: false,
      canFinalize: true,
      canOverride: false,
    })
...
describe('Story 12.3 — SWIFT submit gate reasons', () => {
  it('returns all 3 disabled reasons and enabled state', () => {
    ...
    expect(swiftSubmitDisabledReason('UETR-1', false, false)).toBe(
      'أكمل رفع وثيقة السويفت قبل التسليم',
    )
    expect(swiftSubmitDisabledReason('UETR-1', true, false)).toBe(
      'أكمل رفع طلب تأكيد المصارفة قبل التسليم',
    )
...
describe('Story 12.3 — Banner variants', () => {
  it('selects Director variants with precedence', () => {
    ...
    expect(
      directorBanner(RequestStatus.EXECUTIVE_VOTING_OPEN, UserRole.COMMITTEE_DIRECTOR, true, true),
    ).toBe('ready_to_close')
    expect(
      directorBanner(RequestStatus.EXECUTIVE_VOTING_OPEN, UserRole.COMMITTEE_DIRECTOR, false, true),
    ).toBe('tie_break')
    expect(
      directorBanner(
    ...
    expect(
      directorBanner(RequestStatus.EXECUTIVE_APPROVED, UserRole.COMMITTEE_DIRECTOR, false, false),
    ).toBe('fx_ready')
  })

  it('does not expose Director banners for non-director roles', () => {
    expect(
      directorBanner(RequestStatus.EXECUTIVE_VOTING_OPEN, UserRole.SWIFT_OFFICER, false, false),
    ).toBeNull()
  })
})
...
describe('Story 12.3 — Swift upload access states', () => {
  it('denies wrong role and disallows wrong status', () => {
    expect(swiftUploadAccess(UserRole.COMMITTEE_DIRECTOR, RequestStatus.WAITING_FOR_SWIFT)).toEqual(
      { canAccessPage: false, canUpload: false },
    )
    expect(swiftUploadAccess(UserRole.SWIFT_OFFICER, RequestStatus.SWIFT_UPLOADED)).toEqual({
      canAccessPage: true,
      canUpload: false,
    })
    expect(swiftUploadAccess(UserRole.SWIFT_OFFICER, RequestStatus.WAITING_FOR_SWIFT)).toEqual({
      canAccessPage: true,
      canUpload: true,
    })
  })
