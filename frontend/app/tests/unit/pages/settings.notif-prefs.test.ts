...
vi.mock('../../../stores/auth.store', () => ({
  useAuthStore: () => ({
    get user() { return mockAuthRole ? { role: mockAuthRole } : null },
...
describe('settings — toggleNotifPref calls updateSettings', () => {
  ...
  it('merges existing notification_preferences on toggle', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      success: true,
      data: { language: 'ar', dashboard_view: 'normal', table_density: 'normal', page_size: 25, default_filters: {}, notification_preferences: updated },
    })
