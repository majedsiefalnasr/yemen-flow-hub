...
const fontPreviewFamily = computed(() =>
  `'${themingStore.selectedFontLabel}', 'IBM Plex Sans Arabic', 'Inter Variable', sans-serif`,
)
...
watch(() => ({ shortcutsRequireModifier: themingStore.shortcutsRequireModifier, highContrast: themingStore.highContrast }),
  () => settingsStore.markSectionDirty('theming', 'accessibility'), { deep: true })
...
async function saveAppearanceSettings() {
  ...
  if (success) settingsStore.markSectionClean('theming', 'appearance')
}
...
async function saveBrandingSettings() {
  ...
  if (success) settingsStore.markSectionClean('theming', 'branding')
}

async function saveAccessibilitySettings() {
  const success = await settingsStore.saveSection('theming', {
    shortcutsRequireModifier: themingStore.shortcutsRequireModifier,
    highContrast: themingStore.highContrast,
  }, 'accessibility')
  if (success) settingsStore.markSectionClean('theming', 'accessibility')
}

async function saveGeneralAndBrandingSettings() {
  await Promise.all([
    settingsStore.isSectionDirty('general') ? settingsStore.saveSection('general', generalSettings) : Promise.resolve(true),
    settingsStore.isSectionDirty('theming', 'branding') ? saveBrandingSettings() : Promise.resolve(),
  ])
}
</script>
...
<template>
  <div>
    <PageHeader
      ...
      subtitle="إدارة سير العمل، المظهر، الإشعارات، والأمن"
      ...
      <TabsContent value="general" data-panel="general" class="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div class="flex items-center gap-3">
              ...
              <div>
                <CardTitle>هوية المنصة</CardTitle>
                <CardDescription>الاسم والجهة المشغّلة، الشعار، ولون العلامة</CardDescription>
              </div>
            </div>
          </CardHeader>
          <Separator />
          <CardContent class="space-y-6 pt-6">
            ...
            <!-- Visual branding (merged from separate card) -->
            <div class="flex items-center gap-2">
              <Palette class="h-4 w-4 text-muted-foreground" />
              <h3 class="text-sm font-semibold">الهوية البصرية</h3>
            </div>
            <FieldGroup>
              <Field orientation="horizontal">
                <FieldContent>
                  <FieldTitle>شعار الجهة</FieldTitle>
                  <FieldDescription>SVG أو PNG أو JPG حتى 800×400</FieldDescription>
                </FieldContent>
              ...
              <Field orientation="horizontal">
                <FieldContent>
                  ...
                  <FieldDescription>يُحدّث متغيرات اللون الأساسية في الواجهة</FieldDescription>
                </FieldContent>
            ...
            <div class="flex justify-end">
              <Button
                :disabled="(!settingsStore.isSectionDirty('general') && !settingsStore.isSectionDirty('theming', 'branding')) || settingsStore.saving"
                @click="saveGeneralAndBrandingSettings"
              >
        ...
        <Card>
          <CardHeader>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>وضع الثيم، الخط، وتخطيط العرض</CardDescription>
              </div>
            </div>
          </CardHeader>
          <Separator />
          <CardContent class="space-y-8 pt-6">
            <!-- Theme mode -->
            <div class="space-y-3">
              <Label class="text-sm font-medium">وضع الثيم</Label>
              <p class="text-xs text-muted-foreground">اختر الوضع المناسب أو اربطه بإعدادات النظام.</p>
              <RadioGroup
                    ...
                    <div class="flex h-[150px] items-end overflow-hidden rounded-xl border border-input bg-background transition-all peer-data-checked:border-2 peer-data-checked:border-primary">
                      ...
                      <div v-if="option.value === 'light'" class="grid size-full grid-cols-[48px_1fr] bg-white">
                        <div class="flex flex-col gap-2 border-e border-[#e5e7eb] bg-[#f8fafc] p-3">
                          ...
                          <div class="h-1.5 rounded bg-[#d5dbe5]" />
                          <div class="h-1.5 rounded bg-[#d5dbe5]" />
                          <div class="h-1.5 rounded bg-[#d5dbe5]" />
                        </div>
                        <div class="flex flex-col gap-3 p-4">
                          <div class="flex gap-2">
                            <div class="h-1.5 w-10 rounded bg-[#d5dbe5]" />
                            <div class="ms-auto flex gap-1">
                              <div class="size-1.5 rounded bg-[#d5dbe5]" />
                              <div class="size-1.5 rounded bg-[#d5dbe5]" />
                            </div>
                          </div>
                          <div class="h-8 rounded bg-[#f1f3f5]" />
                          <div class="grid grid-cols-2 gap-2">
                            <div class="h-10 rounded bg-[#f1f3f5]" />
                            <div class="h-10 rounded bg-[#f1f3f5]" />
                          </div>
                        </div>
                      </div>
                      ...
                      <div v-else-if="option.value === 'dark'" class="grid size-full grid-cols-[48px_1fr] bg-[#151820]">
                        <div class="flex flex-col gap-2 border-e border-white/10 bg-[#0f1218] p-3">
                          ...
                          <div class="h-1.5 rounded bg-[var(--color-primary)]" />
                          <div class="h-1.5 rounded bg-[#343a44]" />
                          <div class="h-1.5 rounded bg-[#343a44]" />
                        </div>
                        <div class="flex flex-col gap-3 p-4">
                          <div class="flex gap-2">
                            <div class="h-1.5 w-10 rounded bg-[#343a44]" />
                            <div class="ms-auto flex gap-1">
                              <div class="size-1.5 rounded bg-[#343a44]" />
                              <div class="size-1.5 rounded bg-[#343a44]" />
                            </div>
                          </div>
                          <div class="h-8 rounded bg-[#2a2e3a]" />
                          <div class="grid grid-cols-2 gap-2">
                            <div class="h-10 rounded bg-[#2a2e3a]" />
                            <div class="h-10 rounded bg-[#2a2e3a]" />
                          </div>
                        </div>
                      </div>
            ...
            <!-- Font picker -->
            <div class="space-y-3">
              <Label class="text-sm font-medium">الخط</Label>
              <p class="text-xs text-muted-foreground">ابحث في Google Fonts أو اختر من القائمة الاحتياطية.</p>
              <Popover v-model:open="fontPickerOpen">
                <PopoverTrigger as-child>
                  ...
                  >
                    <span class="truncate">{{ selectedFont?.label || 'اختر خطاً...' }}</span>
              ...
              <div class="rounded-lg border bg-muted/30 px-4 py-3" :style="{ fontFamily: fontPreviewFamily }">
                <p class="text-sm font-medium">منصة إدارة وتمويل الواردات</p>
                <p class="text-xs text-muted-foreground">Yemen Flow Hub — البنك المركزي اليمني</p>
              </div>
              <p class="text-xs text-muted-foreground">
                المصدر: {{ themingStore.fontSource === 'google' ? 'Google Fonts API' : 'قائمة احتياطية' }}
                <span v-if="themingStore.fontsError"> — {{ themingStore.fontsError }}</span>
              </p>
            </div>
            ...
            <!-- Layout mode -->
            <div class="space-y-3">
              <Label class="text-sm font-medium">تخطيط العرض</Label>
              <p class="text-xs text-muted-foreground">اختر بين مساحة عمل كاملة أو عرض محدود.</p>
          ...
          <CardHeader>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>خيارات تُسهّل القراءة والتنقل المتكرر</CardDescription>
              </div>
            </div>
          </CardHeader>
          <Separator />
          <CardContent class="space-y-3 pt-6">
            <div class="flex items-center justify-between rounded-lg border border-border p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-sm font-medium">اختصارات تتطلب مفتاح تعديل</p>
                  <p class="text-xs text-muted-foreground">تجنب تشغيل الاختصارات أثناء الكتابة</p>
                </div>
              </div>
              <Switch
                :model-value="themingStore.shortcutsRequireModifier"
                @update:model-value="themingStore.setShortcutsRequireModifier(Boolean($event))"
              />
            </div>
            <div class="flex items-center justify-between rounded-lg border border-border p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-sm font-medium">تباين عالٍ</p>
                  <p class="text-xs text-muted-foreground">تعزيز قراءة النصوص والحدود في الجلسات الطويلة</p>
                </div>
              </div>
              <Switch
                :model-value="themingStore.highContrast"
                @update:model-value="themingStore.setHighContrast(Boolean($event))"
              />
            </div>

            <div class="flex justify-end pt-2">
              <Button
                :disabled="!settingsStore.isSectionDirty('theming', 'accessibility') || settingsStore.saving"
      ...
      <TabsContent value="workflow" data-panel="workflow" class="space-y-6">
        <Card>
          ...
          <CardContent class="space-y-8 pt-6">
            ...
            <div class="space-y-4">
              <div class="flex items-center gap-2">
                <h3 class="text-sm font-semibold">أعضاء اللجان</h3>
                <Badge variant="secondary" class="text-xs">هيكل تنظيمي</Badge>
              </div>
              <div class="grid gap-5 md:grid-cols-2">
                <FieldGroup>
            ...
            <div class="space-y-4">
              <div class="flex items-center gap-2">
                <h3 class="text-sm font-semibold">إعدادات الاجتماع</h3>
                <Badge variant="secondary" class="text-xs">نصاب ومهل</Badge>
              </div>
              <div class="grid gap-5 md:grid-cols-2">
                <FieldGroup>
          ...
          <CardContent class="space-y-8 pt-6">
            ...
            <div class="space-y-4">
              ...
              <FieldGroup>
                ...
                <p class="text-xs text-muted-foreground">المتغيرات المتاحة: <code class="rounded bg-muted px-1">&#123;&#123;importer&#125;&#125;</code> <code class="rounded bg-muted px-1">&#123;&#123;ref&#125;&#125;</code> <code class="rounded bg-muted px-1">&#123;&#123;amount&#125;&#125;</code> <code class="rounded bg-muted px-1">&#123;&#123;currency&#125;&#125;</code></p>
              </FieldGroup>
            </div>
          ...
          <CardHeader>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>تحديد قنوات التنبيه المفعّلة لأحداث سير العمل</CardDescription>
              </div>
            </div>
          </CardHeader>
          <Separator />
          ...
          <CardHeader>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>معايير الحماية والضبط الإلزامي على مستوى المنصة</CardDescription>
              </div>
            </div>
          </CardHeader>
          <Separator />
