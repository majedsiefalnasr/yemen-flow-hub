...
function severityFor(notification: Notification): Severity {
  ...
  if (
    notification.data?.type === 'request_returned' ||
    notification.data?.type === 'claim_released'
  )
    return 'warning'
  if (
    notification.data?.type === 'request_approved' ||
...
> = {
  critical: {
    icon: XCircle,
    iconWrap:
      'bg-[var(--severity-red)]/12 text-[var(--severity-red)] ring-1 ring-[var(--severity-red)]/25',
    dotClass: 'bg-[var(--severity-red)] text-primary-foreground',
    label: 'عاجل',
  },
  warning: {
    icon: FileText,
    iconWrap:
      'bg-[var(--severity-amber)]/12 text-[var(--severity-amber)] ring-1 ring-[var(--severity-amber)]/25',
    dotClass: 'bg-[var(--severity-amber)] text-foreground',
    label: 'مهم',
  },
  success: {
    icon: CheckCircle2,
    iconWrap:
      'bg-[var(--severity-green)]/12 text-[var(--success)] ring-1 ring-[var(--severity-green)]/25',
  ...
  info: {
    ...
    dotClass: 'bg-[var(--info)] text-foreground',
    label: 'إشعار',
  },
}
...
const selectedNotificationStyle = computed(
  () => SEVERITY_STYLES[selectedNotificationSeverity.value],
)
const selectedNotificationMessage = computed(() =>
  selectedNotification.value ? notificationMessage(selectedNotification.value) : '',
)
const selectedNotificationReference = computed(() =>
  selectedNotification.value ? notificationReference(selectedNotification.value) : null,
)
const selectedNotificationSummary = computed(() =>
  selectedNotification.value ? notificationSummary(selectedNotification.value) : '',
)
...
async function markSelectedRead() {
  const rows = (dataTableRef.value?.table.getSelectedRowModel().rows ??
...
const columns: ColumnDef<Notification>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center gap-2' }, [
        h(
          'div',
          { class: `grid h-8 w-8 shrink-0 place-items-center rounded-full ${style.iconWrap}` },
  ...
  },
  {
    ...
    filterFn: (row, _id, value: string[]) =>
      value.includes(row.original.read_at ? 'read' : 'unread'),
    cell: ({ row }) => {
      const n = row.original
      return n.read_at
  ...
  <div>
    ...
    <div class="relative flex flex-col gap-4">
      <DataTable
        ...
        :row-class="
          (row) =>
            `notification-item notification-row notification-row--${severityFor(row as Notification)} ${!(row as Notification).read_at ? 'notification-row--unread' : ''}`
        "
