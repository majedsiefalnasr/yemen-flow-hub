...
import type { CbyAdminDashboardStats, CbyAdminKpi, CbyAdminWorkflowPressureRow } from '../composables/useDashboard'
...
export function makeKpi(overrides: Partial<CbyAdminKpi> = {}): CbyAdminKpi {
  return { value: 0, delta: 0, severity: 'blue', sparkline: [], drilldown_route: '/requests', ...overrides }
}
...
export function severityColor(severity: CbyAdminKpi['severity']): string {
  ...
  } as Record<string, string>)[severity] ?? 'var(--brand-color)'
}
...
export function slaRiskColor(risk: CbyAdminWorkflowPressureRow['sla_risk']): string {
  ...
  } as Record<string, string>)[risk] ?? 'var(--brand-color)'
}
...
export function trendColor(trend: CbyAdminWorkflowPressureRow['trend']): string {
  ...
  } as Record<string, string>)[trend] ?? 'var(--locked)'
}
