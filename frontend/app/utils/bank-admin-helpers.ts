...
export function calcRejectionRate(stats: (BankAdminDashboardStats & BankAdminDashboardStatsExtended) | null): number {
  if (!stats) return 0
...
export function calcShowHealthStrip(stats: (BankAdminDashboardStats & BankAdminDashboardStatsExtended) | null): boolean {
...
export function calcHealthIssues(stats: (BankAdminDashboardStats & BankAdminDashboardStatsExtended) | null): string[] {
  if (!stats) return []
  const issues: string[] = []
  const rate = calcRejectionRate(stats)
  if (rate > REJECTION_THRESHOLD) issues.push(`معدل الرفض مرتفع: ${rate}%`)
  if ((stats.stalled_at_cby_count ?? 0) > 0) issues.push(`${stats.stalled_at_cby_count} طلب متوقف لدى البنك المركزي`)
